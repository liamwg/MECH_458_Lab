...
const int alum_low = 0;
const int alum_high = 396;
const int steel_low = 397;
const int steel_high = 839;
const int white_low = 840;
const int white_high = 963;
const int black_low = 964;
const int black_high = 1023;
...
const int begin_delay = 226; //we know 250 works fine
volatile int a_delay = 17; // we know 25 works
volatile int a_flag = 0;
volatile int sort_flag = 1;
volatile int belt_flag = 1;
volatile int brake_flag = 1;

const int min_delay = 50;
const int acceleration = 8; //safe is 8
...
ISR(INT0_vect){//rising edge ramp down
	...
	if((PIND & 0x01) == 01){
		...
		TCCR3B |=_BV(WGM32);//Set the Waveform Generation Mode bit description to Clear Timer on Compare Math mode (CTC) only

		OCR3A = 0xFFFF; //Sets output compare register for 65535 cycles = 8.2 s

		TCNT3 = 0x0000; //Sets initial value of timer counter to 0x0000
		
		TIMSK3 |= _BV(OCIE3A); //enable timer 3 compare match a interrupt

		TCCR3B |=_BV(CS32) | _BV(CS30);	//Set scaled clock 8 MHz/1024 -> 8 kHz

		TIFR3 |= _BV(OCF3A);//clear flag and begin timing
		
		while((PIND & 0x01) == 01); //wait until button is released
	}
}
ISR(INT1_vect) { //falling edge pause button
	...
	if((PIND & 0x02) == 0x00 && belt_flag == 1){ //if currently running
		...
		LCDClear();
		LCDWriteStringXY(0,0,"Sorting");
		STATE = 0; // go back to polling stage
		while((PIND & 0x02) == 0x00); // wait until button is released
	}
...
ISR(TIMER3_COMPA_vect){ //once ramp down timer is down, go into end state 
	brake();
...
void aTimer(int count){
	...
	*/
	...
	OCR4A = 0x0064; //Sets output compare register for 100 cycles = 0.1 ms

	TCNT4 = 0x0000; //Sets initial value of timer counter to 0x0000

	TCCR4B |=_BV(CS41);	//Set scaled clock 16MHz/8 -> 2MHz

	TIFR4 |= _BV(OCF4A); //clear timer interrupt flag and begin new timing
	...
	while(k<count){
		...
		if((TIFR4 & 0x02) == 0x02){
			TIFR4 |= _BV(OCF4A); // clear interrupt flag by writing a ONE to the bit
...
void PWM(){
	...
	OCR0A = 0x5A; // set OCRA to something
	//use 0x28 for 25%
	//use 0x4D for 30%
	// use 0x5A for 35%
	// use 0x66 for 40%
...
void stepper_direction(int required_pos){
	...
	if (abs(distance) > 4 && (a_delay > min_delay) ) //accelerating - found max speed to be at a_delay = 7, 
	a_delay -= acceleration;
	if (abs(distance) < 10 && (a_delay < 200) ) //braking
	a_delay += 10;
